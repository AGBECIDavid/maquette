...
import { catIco, catLabel, CATEGORIES, CLAIM_STATUS, CLAIM_CATEGORIES, TOKEN_TTL } from "../lib/data.js";
  ...
  return (
    <>
      <div className="pagehead"><div><h1>Bonjour {me.name.split(" ")[0]}</h1>
        <p>Votre budget Ticket Tout et vos dernières opérations.</p></div></div>
      ...
      <div className="paycard">
        ...
        <div className="paycard__bal">
          <div className="lb">À dépenser</div>
...
function Payer({ me }) {
  ...
  useEffect(() => {
    ...
    try { drawQR(canvas.current, "TT1|" + tok.token, 236); }
  ...
  const emettre = async () => {
    try {
      ...
      toast("info", "Jeton émis", "Valable 30 minutes, un seul encaissement.");
    ...
    <>
      <div className="pagehead"><div><h1>Payer</h1>
        <p>Présentez ce code au partenaire. Il saisit le montant, vous êtes débité à la validation.
          Le jeton reste valable 30 minutes : le temps de faire la queue.</p></div></div>
            ...
            <div className="tokenbox">
              ...
              <div className="tokenbox__row">
                <div className="tokenbox__v mono">
                  <span className="tokenbox__pfx">TT1|</span>
                  {tok.token.match(/.{1,4}/g).map((g, i) => <span key={i}>{g}</span>)}
                </div>
                <CopyButton className="tokenbox__copy" value={"TT1|" + tok.token} label="Copier" />
              </div>
            </div>
          ) : null}
  ...
  const fiche = async id => {
    ...
    modal.open(p.name, (
      <>
        ...
        <hr className="sep" />
        <Note>Présentez votre QR code Ticket Tout en caisse. Le partenaire saisit le montant,
  ...
  return (
    <>
      <div className="pagehead"><div><h1>Partenaires</h1>
        <p>Les établissements où votre budget Ticket Tout est accepté.</p></div></div>
